...
#define LK(x)                    \
        ...
        SM4_ROTL((x), 23))
//Encrypt F'
#define ENC_K(x0, x1, x2, x3, x4, i)         \
        x4 = x1 ^ x2 ^ x3 ^ *(sm4_ck + i);   \
        x4 = SBOX(x4);                       \
        x4 = x0 ^ LK(x4);                    \
        *(rk + i) = x4
//Decrypt F'
#define DEC_K(x0, x1, x2, x3, x4, i)         \
        x4 = x1 ^ x2 ^ x3 ^ *(sm4_ck + i);   \
        x4 = SBOX(x4);                       \
        x4 = x0 ^ LK(x4);                    \
        *(rk + 31 - i) = x4
...
//Generate SM4 encrypt key
void sm4_enc_key(OUT uint32_t *rk,
                    IN const unsigned char *key){
    
    uint32_t x0 = 0, x1 = 0, x2 = 0, x3 = 0, x4 = 0;
    x0 = GET(key     ) ^ sm4_fk[0];
    x1 = GET(key  + 4) ^ sm4_fk[1];
    x2 = GET(key  + 8) ^ sm4_fk[2];
    x3 = GET(key + 12) ^ sm4_fk[3];
#undef ROUND
#define ROUND ENC_K
    ROUNDS(x0, x1, x2, x3, x4);
}

//Generate SM4 decrypt key
void sm4_dec_key(OUT uint32_t *rk,
                    IN const unsigned char *key){
